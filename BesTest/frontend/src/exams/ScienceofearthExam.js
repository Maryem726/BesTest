...
const handleChange = (e) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () =>{
      setLoading(true);
      try {
        const {data: response} = await  Axios.get(`/examen/examsBysciences/${user._id}`)
        setData(response);
      } catch (error) {
      ...
      <div className="search-box" >
          ...
          {data.slice(0, visible).filter(el=>el.title.toLowerCase().includes(Search.toLowerCase())).map((post, index) => (
              <PostContainer key={index} featured={post.subject}>
                ...
                <td><a href={`assets/uploads/${post.type}`}download ><Image imageSrc= "https://clg-lapierre-stjeandugard.ac-montpellier.fr/sites/clg-lapierre-stjeandugard/files/source_1.gif" /></a></td>
                  <Info>
...
export default ScienceofearthExam;
