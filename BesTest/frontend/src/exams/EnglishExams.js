...
const handleChange = (e) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () =>{
      setLoading(true);
      try {
        const {data: response} = await  Axios.get(`/examen/getbyname/byEnglish/${user._id}`)
        setData(response);
      } catch (error) {
  ...
  {!loading && (
    <AnimationRevealPage>
     ...
     <Hero
        heading={<> <HighlightedText>English Exams</HighlightedText></>}
        
        description=""
        imageSrc="https://math-center.org/_nuxt/img/fishing-on-a-cloud.825cb6e.svg?fbclid=IwAR0vdiSPH0uG8ukTROhDMWNqEHZZ4PMlKGKreUR4eK4TdE2wz-0oMG3K8VI"
      ...
      <div className="search-box" >
          ...
          {data.slice(0, visible).filter(el=>el.title.toLowerCase().includes(Search.toLowerCase())).map((post, index) => (
              <PostContainer key={index} featured={post.subject}>
                ...
                <td><a href={MyPdf}download ><Image imageSrc= "https://media.baamboozle.com/uploads/images/220206/1614662730_31585_gif-url.gif" /></a></td>
                  <Info>
...
const getPlaceholderPost = () => ({
  imageSrc:
  "https://media.baamboozle.com/uploads/images/220206/1614662730_31585_gif-url.gif",
  category: "exam",
  date: "April 19, 2020",
  title: "exam",
  url: ""
});
export default EnglishExams;