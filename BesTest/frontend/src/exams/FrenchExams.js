...
const handleChange = (e) => {
  ...
  const HighlightedText = tw.span`bg-indigo-600 text-white px-4 transform -skew-x-12 inline-block`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () =>{
      ...
      setLoading(true);
      try {
        const {data: response} = await  Axios.get(`/examen/examsByfrench/${user._id}`)
        setData(response);
      } catch (error) {
  ...
  {!loading && (
    <AnimationRevealPage>
      <Container tw="bg-gradient-to-b from-indigo-600  via-indigo-100 to-white relative -mx-8 -mt-8 pt-8 px-8">       

     <Hero
        heading={<> <HighlightedText>French Exams</HighlightedText></>}
        
        description=""
        imageSrc="https://images.squarespace-cdn.com/content/v1/5f4677c11310ba489fc553b5/1610648226741-XGEGGBYP95U00JHZLA1C/French+Boy.png?fbclid=IwAR3tLHOGK-nKf-s1o6GgiTwO1nefvsVr1vKGn8IMSaQXv3VB9ErMDCiN3dU"
